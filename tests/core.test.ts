...
describe("discoverDevcontainerConfig", () => {
  ...
  test("finds a devcontainer file in the requested subpath", async () => {
    ...
    await writeFile(
      ...
      `{ "image": "mcr.microsoft.com/devcontainers/typescript-node:22-trixie" }`,
    );
    ...
    expect(discovered.config.image).toBe("mcr.microsoft.com/devcontainers/typescript-node:22-trixie");
  });
